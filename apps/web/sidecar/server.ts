...
function createDaemonProxyHandler(
  ...
  // BEAM bridge — when BEAM_DAEMON_URL is set, intercept run/event
  // routes BEFORE the daemon proxy. The bridge handles them by
  // talking to the BEAM daemon over WebSocket and translating events
  // back to ChatSseEvent shape; everything else falls through to the
  // JS daemon proxy or Next.js handler exactly as upstream does.
  // No-op when BEAM_DAEMON_URL is unset.
